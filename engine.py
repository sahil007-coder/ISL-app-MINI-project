...
# CONFIDENCE SETTINGS: 0.90 matlab 90% accuracy chahiye bolne ke liye
CONFIDENCE_THRESHOLD = 0.90
BUFFER_SIZE = 12  # Thoda chhota buffer taaki 90% jaldi achieve ho
...
class ISLEngine:
    ...
    def get_frame(self):
        ...
        if results.hand_landmarks:
            ...
            hands_list.sort(key=lambda x: x[0])  # Left-to-Right Sort
            ...
            for i, (wrist_x, hand_lms) in enumerate(hands_list):
                ...
                if i == 0:
                    left_data = temp_points
                else:
                    right_data = temp_points

            # Prediction
            input_tensor = torch.tensor(left_data + right_data, dtype=torch.float32).unsqueeze(0).to(self.device)
            if self.device.type == "cuda": input_tensor = input_tensor.half()
            ...
            if len(self.prediction_buffer) == BUFFER_SIZE:
                ...
                # Check if most predictions agree AND confidence is > 90%
    ...
    def get_stats(self):
        return {"sign": self.current_sign, "confidence": round(float(self.current_confidence), 2),
                "history": " ".join(self.sentence_history[-10:]), "camera_active": self.camera_active}
